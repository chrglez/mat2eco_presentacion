...
- **Grupo 1** [Julián Andrada](mailto:julian.andrada@ulpgc.es)
...
.pull-left[
- `r emo::ji("book")` **Teoría y ejemplos** 3 horas; *Aula A-2.1 y telepresencial*
    - Guiones de clase
    - Relaciones de problemas

- `r emo::ji("screen")` **Prácticas: Ejercicios** 1  hora; *Telepresencial*
...
tmp <- as.data.frame(matrix(day(seq.Date(ymd("2022/01/31"),ymd("2022/05/22"), by = "day")), ncol =7, byrow= TRUE))
tmp[,6:7] <- NULL
names(tmp) <- c("Lunes","Martes","Miércoles","Jueves","Viernes")

rtmp <- as.data.frame(matrix(c(rep(NA,5),1:5,rep(NA,5),4:8,rep(NA,5)), ncol = 5,byrow=TRUE))
...
plan <- data.frame(
  SEMANA <- c(as.character(1:10),"Semana Santa",as.character(11:15),rep(NA,5)),
  MES <- c("enero/febrero",rep("febrero",3),"febrero/marzo",rep("marzo",3),"marzo/abril",rep("abril",3),"abril/mayo",rep("mayo",3),"...","junio","...","julio","...")
)


aulas <- data.frame(
  `Aula A-2.1` = c("Presentación/Tema1", rep("Tema 1",2), rep("Tema 2",3),   rep("Tema 3",4),NA, "EC1 21/04/2022", rep("Tema 4",4),NA, "Concocatoria ordinaria <a href='https://feet.ulpgc.es/sites/default/files/wp-content/uploads/2021/07/calendario_examenes_ge_2021-22_aprobado_en_junta_090721.pdf'><i class='fas fa-external-link-alt'></i></a>",NA,"Concocatoria extraordinaria  <a href='https://feet.ulpgc.es/sites/default/files/wp-content/uploads/2021/07/calendario_examenes_ge_2021-22_aprobado_en_junta_090721.pdf'><i class='fas fa-external-link-alt'></i></a>",NA),
  `Práctica` = c(rep("Práctica 0",2), rep("Práctica 1",2), rep("Práctica 2",3),  rep("Práctica 3",3),NA, "EC1p 19/04/2022", rep("Práctica 4",3),"EC2p 17/05/2022",NA, "02/06/2022" ,NA,"06/07/2022",NA)
  )

cal <- cbind(plan,fechas,aulas)
names(cal) <- c("SEMANA", "MES","Lunes","Martes","Miércoles","Jueves","Viernes", "Aula A-2.1", "Práctica (D.03)")
...
cal %>%
  ...
  column_spec(4,  background = ifelse(grepl("^EC",cal$Práctica) | grepl("Concocatoria ordinaria",cal$`Aula A-2.1`), "#31c854", "white"), color = ifelse(grepl("^EC",cal$Práctica) | grepl("Concocatoria ordinaria",cal$`Aula A-2.1`), "white", "black")) %>%
  column_spec(5,  background = ifelse(grepl("Concocatoria extraordinaria",cal$`Aula A-2.1`), "#31c854", "white"), color = ifelse(grepl("Concocatoria extraordinaria",cal$`Aula A-2.1`), "white", "black")) %>% 
   
    column_spec(6,  background = ifelse(grepl("^E",cal$`Aula A-2.1`) , "#31c854", "white"), color = ifelse(grepl("^E ",cal$`Aula A-2.1`) , "white", "black")) %>%
  column_spec(8,  background = ifelse(grepl("^[EC]",cal$`Aula A-2.1`), "#31c854", "white"), color = ifelse(grepl("^[EC]",cal$`Aula A-2.1`), "white", "black")) %>%
      column_spec(9,  background = ifelse(grepl("^[E1-9]",cal$Práctica), "#31c854", "white"), color = ifelse(grepl("^[E1-9]",cal$Práctica), "white", "black")) %>%
